...
def create_role(role_data: RoleCreateRequest, db: Session = Depends(get_db_session)):
    """Create a new role"""
    try:
...
def get_roles(skip: int = 0, limit: int = 100, db: Session = Depends(get_db_session)):
    """Get all roles"""
...
def update_role(
    role_id: int, role_data: RoleUpdateRequest, db: Session = Depends(get_db_session)
):
    """Update role"""
    try:
...
def get_role_by_name(name: str, db: Session = Depends(get_db_session)):
    """Get role by name"""
...
def get_active_roles(db: Session = Depends(get_db_session)):
...
def activate_role(role_id: int, db: Session = Depends(get_db_session)):
    """Activate role"""
...
def deactivate_role(role_id: int, db: Session = Depends(get_db_session)):
    """Deactivate role"""
