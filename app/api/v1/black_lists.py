...
def create_black_list(
    black_list_data: BlackListCreateRequest, db: Session = Depends(get_db_session)
):
...
def get_black_list(black_list_id: int, db: Session = Depends(get_db_session)):
    """Get black list entry by ID"""
    black_list = black_list_service.get_by_id(db, black_list_id)
    if not black_list:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Black list entry not found"
        )
    return black_list
...
def update_black_list(
    ...
    try:
        black_list = black_list_service.update_black_list(
            db, black_list_id, black_list_data
        )
        if not black_list:
            raise HTTPException(
...
def delete_black_list(black_list_id: int, db: Session = Depends(get_db_session)):
...
def get_black_list_by_name(name: str, db: Session = Depends(get_db_session)):
...
def get_active_black_lists(db: Session = Depends(get_db_session)):
...
def get_black_lists_by_category(category: int, db: Session = Depends(get_db_session)):
