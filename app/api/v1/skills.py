...
def create_skill(skill_data: SkillCreateRequest, db: Session = Depends(get_db_session)):
    """Create a new skill"""
...
def get_skills(skip: int = 0, limit: int = 100, db: Session = Depends(get_db_session)):
    """Get all skills"""
...
def get_active_skills(db: Session = Depends(get_db_session)):
...
def get_skill(skill_id: int, db: Session = Depends(get_db_session)):
    """Get skill by ID"""
...
def update_skill(
    skill_id: int, skill_data: SkillUpdateRequest, db: Session = Depends(get_db_session)
):
    """Update skill"""
...
def delete_skill(skill_id: int, db: Session = Depends(get_db_session)):
    """Soft delete skill"""
...
def deactivate_skill(skill_id: int, db: Session = Depends(get_db_session)):
    """Deactivate skill"""
...
def search_skills_by_name(name: str, db: Session = Depends(get_db_session)):
