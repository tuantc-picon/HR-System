...
from services.job_requirement_certificate_service import (
    job_requirement_certificate_service,
)
...
from schema.response.job_requirement_certificate_schemas import (
    JobRequirementCertificateResponse,
)
...
@router.post(
    "/",
    response_model=JobRequirementCertificateResponse,
    status_code=status.HTTP_201_CREATED,
)
def create_job_requirement_certificate(
    data: JobRequirementCertificateCreateRequest, db: Session = Depends(get_db_session)
):
    """Create a new job requirement certificate association"""
    try:
        association = (
            job_requirement_certificate_service.create_job_requirement_certificate(
                db, data
            )
        )
        return association
...
def get_job_requirement_certificates(
    ...
    associations = job_requirement_certificate_service.get_all(
...
def update_job_requirement_certificate(
    ...
    association = (
        job_requirement_certificate_service.update_job_requirement_certificate(
...
@router.get(
    ...
    response_model=List[JobRequirementCertificateResponse],
)
def get_certificates_by_job_requirement(
    ...
    associations = (
        job_requirement_certificate_service.get_certificates_by_job_requirement(
...
def get_job_requirements_by_certificate(
    certificate_id: int, db: Session = Depends(get_db_session)
):
    """Get all job requirements that require a specific certificate"""
    associations = (
        job_requirement_certificate_service.get_job_requirements_by_certificate(
            db, certificate_id
        )
    )
    return associations
...
@router.delete(
    "/job-requirement/{job_requirement_id}/certificate/{certificate_id}",
    status_code=status.HTTP_204_NO_CONTENT,
)
def remove_certificate_from_job_requirement(
    job_requirement_id: int, certificate_id: int, db: Session = Depends(get_db_session)
):
    """Remove a certificate from a job requirement"""
    success = (
        job_requirement_certificate_service.remove_certificate_from_job_requirement(
            db, job_requirement_id, certificate_id
        )
    )
    if not success:
        raise HTTPException(
