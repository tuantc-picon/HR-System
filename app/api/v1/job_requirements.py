...
@router.post(
    "/", response_model=JobRequirementResponse, status_code=status.HTTP_201_CREATED
)
def create_job_requirement(
    ...
    try:
        job_requirement = job_requirement_service.create_job_requirement(
            db, job_requirement_data
        )
...
def get_job_requirement(job_requirement_id: int, db: Session = Depends(get_db_session)):
    """Get job requirement by ID"""
    job_requirement = job_requirement_service.get_by_id(db, job_requirement_id)
    if not job_requirement:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Job requirement not found"
        )
    return job_requirement
...
def update_job_requirement(
    ...
    try:
        job_requirement = job_requirement_service.update_job_requirement(
            db, job_requirement_id, job_requirement_data
        )
        if not job_requirement:
            raise HTTPException(
...
def get_requirements_by_job(job_id: int, db: Session = Depends(get_db_session)):
